        ...
        if ( $access_token ) {
           ...
           $csrf_token = LoginWithAmazonUtility::getCsrfToken();
            ...
            if ( $csrf_token && LoginWithAmazonUtility::verifyCsrfToken($csrf_token) ) {
				...
				$requestData = LoginWithAmazonUtility::getDataFromAccessToken($access_token);

                if ($email) {
                    // Find or create an Amazon user
                    $user = LoginWithAmazonUtility::findOrCreateUserByEmail($email,$requestData);
					...
					<script>
					jQuery(function() {
						errmsg = "<?php echo $errorMessa; ?>";
						jQuery('.container.login').prepend("<p class='login-msg'>"+ errmsg +" </p>");
