...
class LoginWithAmazonUtility {
    ...
    public static function getEmailFromAccessToken($accessToken) {
	 ...
	 public static function getDataFromAccessToken($accessToken) {
        $result = self::json_curl_wrapper('https://api.amazon.com/auth/o2/tokeninfo?access_token=' . urlencode($accessToken));
        if (!isset($result['aud']) || $result['aud'] != get_option('loginwithamazon_client_id')) {
            return false;
        }
     ...
     */
    public static function findOrCreateUserByEmail($email,$requestData) {
            ...
            if (self::isAmazonUser($user)) {
               ...
               // add_action( 'login_form', array('LoginWithAmazonUtility', 'addReregisterFieldsToForm') );
				...
				<script src='//code.jquery.com/jquery-1.10.2.js'></script>
				 <script>
					jQuery(function() {
						formhtml1 = "<?php echo self::getCsrfToken(); ?>";
						formhtml2 = "<?php echo self::getAcessToken(); ?>";
					   
						formHtml = '<input type="hidden" name="amazonLogin" value="1">';
						formHtml += '<input type="hidden" name="access_token" value="'+ formhtml2 +'">';
						formHtml += '<input type="hidden" name="state" value="'+ formhtml1 +'">';
						formHtml += '<input type="hidden" name="loginwithamazon_reregister" value="1">';
						jQuery('#loginform').prepend(formHtml);
		...
		$first_name = isset($requestData['name'])?$requestData['name']:'';
		$postal_code = isset($requestData['postal_code'])?$requestData['postal_code']:'';
